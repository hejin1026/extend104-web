...
class Table(object):
    ...
    def build(self, request, profile):
        self.order_by = request.args.get('order_by', '')
        self.hiddens = profile.get(self.profile_hiddens_key, '')

        try:
            page = int(request.args.get('page', Table.DEFAULT_PAGE))
        except Exception, e:
            print e
            page = Table.DEFAULT_PAGE
        
        per_page = int(profile.get(self.profile_perpage_key, 0))
        if per_page == 0:
            try:
                per_page = int(request.args.get('per_page', Table.DEFAULT_PER_PAGE))
            except Exception, e:
                print e
                per_page = Table.DEFAULT_PER_PAGE
...
def make_table(req, query, table_cls, profile=None):
    """ 一个方便使用 Table 的函数 """
    from tango.models import Profile
    if profile is None:
        profile = Profile.get(table_cls._meta.profile)
    table = table_cls(query)
    TableConfig(req, profile).configure(table)
    return table
